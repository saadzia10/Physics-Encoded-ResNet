...
FILE* SegLearn::getKarmaFilename(tTrack* track, tSituation *s, int driverindex)
{
	...
	switch (s->_raceType) {
		case RM_TYPE_RACE:
			fd = tryKarmaFilename(buffer, sizeof(buffer), "%sdrivers/fixed/%d/race/%s.karma", driverindex, tbuf, s->_raceType == RM_TYPE_RACE);
			if ( fd != NULL) {
				return fd;
			} // not found, try the next.
		case RM_TYPE_QUALIF:
			fd = tryKarmaFilename(buffer, sizeof(buffer), "%sdrivers/fixed/%d/qualifying/%s.karma", driverindex, tbuf, s->_raceType == RM_TYPE_QUALIF);
			if ( fd != NULL) {
				return fd;
			} // not found, try the next.
		case RM_TYPE_PRACTICE:
			fd = tryKarmaFilename(buffer, sizeof(buffer), "%sdrivers/fixed/%d/practice/%s.karma", driverindex, tbuf, s->_raceType == RM_TYPE_PRACTICE);
			if ( fd != NULL) {
				return fd;
			} // not found, try the next.		
		default:
