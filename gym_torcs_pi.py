...
class PIEnv(TorcsEnv):
    ...
    def agent_to_torcs(self, actions):
        torcs_action = {'steer': actions['steer']}

        if self.throttle is True:  # throttle action is enabled
            torcs_action.update({'accel': actions['accel']})

        if self.brake_change is True:
            torcs_action.update({'brake': actions['brake']})

        if self.gear_change is True: # gear change action is enabled
            torcs_action.update({'gear': actions['gear']})

        if 'clutch' in actions.keys():
            torcs_action.update({'clutch': actions['clutch']})

        return torcs_action

    def step(self, actions, continuous=True, normalize=True):
        client = self.client

        this_action = self.agent_to_torcs(actions)
        ...
        if 'clutch' in this_action.keys():
            action_torcs['clutch'] = this_action['clutch']
        ...
        return obs, obs_pre, client.R.d['meta']
