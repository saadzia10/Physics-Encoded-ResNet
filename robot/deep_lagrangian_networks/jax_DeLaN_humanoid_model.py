...
def mass_matrix_fn(q, n_dof, shape, activation, epsilon, shift):
    ...
    mat_idx = jax.numpy.index_exp[..., mat_idx[0], mat_idx[1]]
...
def dynamics_model(params, key, q, qd, qdd, lagrangian, n_dof):
    argnums = [2, 3]
    vmap_dim = (None, None, 0, 0)
    batch_matmul = jax.vmap(jnp.matmul, (0, 0))
    ...
    qdd_pred = batch_matmul(batch_inverse(d2Ld2qd + 1.e-4 * jnp.eye(n_dof)), (tau_pred - batch_matmul(d2L_dqddq, qd) + dLdq))
...
def loss_fn(params, q, qd, qdd, lagrangian, n_dof, norm_qdd):
    qdd_pred, tau_pred, H_pred, dHdt_pred = dynamics_model(params, None, q, qd, qdd, lagrangian, n_dof)
