...
class PINN_Trainer:
        ...
        {"mse_loss_E0": [],
         "mse_loss_rb": [], "mse_temp_loss": []})
            ...
            for batch in train_data_loader:
                ...
                nee_pred, dT_dt_pred, k_pred, f_pred, z = model(x, b, k, T)
                # Extract E0 and rb predictions
                E0_pred, rb_pred = k_pred[:, 0], k_pred[:, 1]
                E0, rb = k[:, 0].view((-1, 1)), k[:, 1].view((-1, 1))
        
                preds = [E0_pred, rb_pred, dT_dt_pred]
                gt = [E0, rb, dtemp]
                # Compute loss
                losses = self.loss_function_k(preds, gt, loss_fn)
                mse_loss_E0, mse_loss_rb, mse_loss_temp = losses[0], losses[1], losses[2]
        
                loss = mse_loss_E0 + mse_loss_rb + mse_loss_temp
    ...
    def train_2(self, model, loss_fn, train_data_loader, test_data_loader, num_epochs,
        ...
        while epoch < num_epochs:
            ...
            for batch in train_data_loader:
                ...
                mse_loss_nee, mse_loss_E0, mse_loss_rb, mse_loss_temp, mse_loss_f = self.loss_function(
                    nee_pred, nee, dT_dt_pred, dtemp, k_pred, k, f_pred, f,
                    loss_fn)
        
                mse_loss = mse_loss_nee + mse_loss_E0 + mse_loss_rb + mse_loss_temp + mse_loss_f
    ...
    def loss_function(self, nee_pred, nee_true, temp_pred, temp_true, E0_rb_pred,
        ...
        return loss_nee, loss_E0, loss_rb, temp_loss, f_loss

    def predict(self, model, test_data_loader):
        preds = DotMap(
        {"nee": [], "E0": [], "rb": [], "dtemp": [], "f": [], "z": [], "noise": [], "noise_mus": [],
         "noise_stds": []})
...
class PINN(nn.Module):
    def __init__(self, input_dim, latent_dim, encoder_dims, decoder_dims, noise_dims=[4], activation=nn.ReLU, device="cuda"):
