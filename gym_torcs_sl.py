...
class SLEnv(TorcsEnv):
    def __init__(self, vision=False, throttle=True, gear_change=True, brake_change=True, race_config_path=None, render=False, clutch_change=True):
        ...
        self.clutch_change = clutch_change

    def agent_to_torcs(self, u):
        torcs_action = {'steer': u[3]}

        if self.throttle is True:  # throttle action is enabled
            torcs_action.update({'accel': u[0]})

        if self.brake_change is True:
            torcs_action.update({'brake': u[1]})

        if self.clutch_change is True: # gear change action is enabled
            torcs_action.update({'clutch': u[2]})
